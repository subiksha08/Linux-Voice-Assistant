    ...
    def TaskExecution(self, var):
         ...
         while not self.stop_event.is_set():  
            ...
            if osName!="Windows":
                ...
                if "search file" in self.query:
                    speak("which file u want to search", var)    
                    directory = eel.js_inputBox("Enter the directory to search")()
                    pattern = eel.js_inputBox("Enter the file pattern to search (e.g., *.txt, myfile*.py): ")()
                    # Search for files
                ...
                if 'modify packages' in self.query:
                    ...
                    if command:
                        if any(keyword in command for keyword in ["install", "remove", "update", "upgrade"]):
                            handle_package_management(command)
                        else:
                            speak("Sorry, I can only help with installing, removing, updating, or upgrading packages.",var)
            ...
            if 'activity' in self.query:
                ...
                if osName == 'Windows':
                    musicPath = f"C:\\Users\\{userName}\\Music"
                else:
            ...
            if 'getTime' in Queries:
                ...
                speak(f"Sir, the time is {strTime}", var)
