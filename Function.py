...
def YoutubeUrl(search_keyword,var):
    ...
    try:
        ...
        speak(f"Sorry sir I cannot find that {search_keyword.replace('+',' ')} you said",var)
        return '0'
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    speak('Sorry sir some Error  acquired so I playing Song in the Vlc Player',var)
                    command = 'vlc-ctrl play -p"'+"'"+song_uri+"'"+'"'
                    os.system(command)
                    eel.js_textbox(f"playing {song_uri.split(f'{musicPath}/')[-1]}")
            ...
            speak('Sorry sir some Error  acquired',var)
            return  None
 
import fnmatch

def search_files(directory="", pattern="",var='0'):
    matches = []
    ...
    if matches:
        print(matches)
        speak("Files found:",var)
        for file in matches:
            os.popen(file)
    else:
        speak("No files found matching the pattern.",var)

def handle_package_management(command='' ,var='0'):
    if "install" in command:
        package = command.replace("install", "").strip()
        speak(f"Installing {package}",var)
        os.system(f"sudo apt-get install -y {package}")
        speak(f"{package} has been installed.",var)
    
    elif "remove" in command:
        package = command.replace("remove", "").strip()
        speak(f"Removing {package}",var)
        os.system(f"sudo apt-get remove -y {package}")
        speak(f"{package} has been removed.",var)
    
    elif "update" in command:
        speak("Updating package list",var)
        os.system("sudo apt-get update")
        speak("Package list has been updated.",var)
    
    elif "upgrade" in command:
        speak("Upgrading all packages",var)
        os.system("sudo apt-get upgrade -y")
        speak("All packages have been upgraded.",var)